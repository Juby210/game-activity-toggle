...
module.exports = class GameActivityToggle extends Plugin {
    async startPlugin() {
        ...
        const enabledIcon = React.createElement('svg', {
        ...
        const disabledIcon = React.createElement('svg', {
            viewBox: '0 0 24 24', width: '20', height: '20'
        }, React.createElement('path', {
            style: { fill: '#B9BBBE' },
        ...
        const Account = getOwnerInstance(await waitFor(`.${classes.container}:not(#powercord-spotify-modal)`))
        inject('game-activity-toggle', Account.__proto__, 'render', (_, res) => {
            ...
            if (!r) return res
            showCurrentGame = getModule(['showCurrentGame'], false).showCurrentGame

            r.props.children.unshift(React.createElement(r.props.children[0].type, {
                icon: () => showCurrentGame ? enabledIcon : disabledIcon,
                onClick: () => {
                    showCurrentGame = !showCurrentGame
                    settings.updateLocalSettings({ showCurrentGame })
                    settings.updateRemoteSettings({ showCurrentGame })
                    forceUpdateElement(`.${classes.container}:not(#powercord-spotify-modal)`)
                },
                tooltipText: Messages.SHOW_CURRENT_GAME
            }))
        ...
        getModule(['showCurrentGame'], false).addChangeListener(this.changeListener = () => {
            if (showCurrentGame != getModule(['showCurrentGame'], false).showCurrentGame)
                forceUpdateElement(`.${classes.container}:not(#powercord-spotify-modal)`)
        })
    }
